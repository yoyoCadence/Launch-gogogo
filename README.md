...
小劇場素材放在 `assets/theater/`。後續接手產出素材時，請先看 [`task.md`](./task.md) 的 `Theater Animation Asset Spec`，那裡定義了完整動畫素材規格、命名、資料夾位置與第一階段原型範圍。
...
2. 加入附近喜好店家推薦，依使用者設定過的 Google Maps / 店家連結與目前地理位置，推薦當下附近可吃的店。
   ...
   - 附近篩選：用目前位置計算喜好店家距離，優先顯示營業餐別符合、距離近、評分高、最近較少吃的店家。
